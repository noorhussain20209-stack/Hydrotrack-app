      ...
      if (!await Permission.notification.isGranted) {
        ...
        if (!status.isGranted) {
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(
                content: Text('Please grant notification permission in settings'),
              ),
            );
          }
          return;
                  ...
                  FutureBuilder<PermissionStatus>(
                    ...
                    builder: (context, snapshot) {
                      ...
                      return Text(
                        status.isGranted
                          ? 'Notifications: ON'
                          : 'Notifications: OFF (tap bell icon to enable)',
                        style: TextStyle(
                          fontSize: 12,
